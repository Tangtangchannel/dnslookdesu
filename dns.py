...
def main():
    ...
    parser.add_argument("-mc", "--minecraft", action="store_true", help="查询 Minecraft 服务器的 SRV 记录，将自动添加MC前缀")
    parser.add_argument("-d", "--dns", type=str, help="要查询的 DNS 服务器（可选），默认为标准 DoH 请求 URL。例如 8.8.8.8 为 'https://8.8.8.8/dns-query'")
    ...
    if args.minecraft:
        srv_domain = f"_minecraft._tcp.{args.domain}"
        record_type = "SRV"
